...
if __name__ == "__main__":
    api = TwitterClient()
    tweets = api.get_tweets(query = 'rape', count = 200)
    ...
    for tweet in ptweets:
        ...
        word_list_new = t_check.split();
        for i in word_list_new if i.lower() in swear_words:
        	counter_offensive = counter_offensive + 1
    ...
    total_non_off_tweets = custom_pos_noff_count + custom_neg_noff_count
    total_off_tweets = custom_pos_off_count + custom_neg_off_count

    print("Total number of offensive words in tweets are: " + counter_offensive)

    print("From total " + (len(ptweets)+len(ntweets)) + " tweets (negative plus positive), " + len(total_off_tweets) + " are offensive and " + len(total_non_off_tweets) + " are non offensive")
